...
// 2. Add a function to display forecast data
...
// 3. Helper function to group forecasts by day
...
// 4. Modify the fetchWeather and fetchWeatherByCity functions to also fetch forecast
const originalFetchWeather = fetchWeather;
fetchWeather = function() {
    originalFetchWeather();
    const city = document.getElementById('cityInput').value.trim();
    if (city) {
        // Split out country if present
        const parts = city.split(',');
        const cityName = parts[0].trim();
        let country = 'NO'; // Default

        if (parts.length > 1) {
            country = parts[parts.length - 1].trim();
        }

        fetchForecast(cityName, country);
    }
};

const originalFetchWeatherByCity = fetchWeatherByCity;
fetchWeatherByCity = function(city) {
    originalFetchWeatherByCity(city);
